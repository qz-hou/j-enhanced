...
def validate_scaffold_controls(decision_mode: str, compute_jacobian: bool):
    ...
    if decision_mode == "jacobian":
        raise NotImplementedError(
            "decision_mode='jacobian' is scaffolded but Jacobian "
...
def _gen(prompts, model):
    ...
    for i in range(0, len(outputs), 2):
        ...
        jacobian_score = samp_output.uncertainty.get('jacobian_score', None)
        jacobian_available = samp_output.uncertainty.get(
...
if __name__=="__main__":
    ...
    model = LLM(
        ...
        enable_prefix_caching=not args.compute_jacobian,
        enforce_eager=True
    )
