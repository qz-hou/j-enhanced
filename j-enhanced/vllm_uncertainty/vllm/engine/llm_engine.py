...
class LLMEngine:
    ...
    def from_engine_args(
        ...
        setattr(engine, "decision_mode", engine_args.decision_mode)
        setattr(engine, "compute_jacobian", engine_args.compute_jacobian)
        return engine
    ...
    def _process_model_outputs(
        ...
        for scheduled_seq_group, seq_group_meta in zip(scheduled_seq_groups,
                                                       seq_group_metadata_list):
            seq_group = scheduled_seq_group.seq_group
            seq_group.maybe_set_first_token_time(now)
            request_output = RequestOutputFactory.create(seq_group)
            self._attach_jacobian_score(request_output, seq_group_meta)
    ...
    def compute_uncertainty(self, request_output: RequestOutput):
        ...
        if len(request_output.outputs) > 1:
            ...
            uncertainty_dict['jacobian_score'] = request_output.outputs[0].uncertainty.get(
                'jacobian_score', None)
            uncertainty_dict['jacobian_available'] = request_output.outputs[0].uncertainty.get(
                'jacobian_available', False)
        setattr(request_output, 'uncertainty', uncertainty_dict)

    def _attach_jacobian_score(self, request_output: Union[RequestOutput,
                                                           EmbeddingRequestOutput],
                               seq_group_meta: SequenceGroupMetadata) -> None:
        if (not self.compute_jacobian or
                not isinstance(request_output, RequestOutput) or
                not request_output.finished or len(request_output.outputs) != 1):
            return
        driver_worker = getattr(self.model_executor, "driver_worker", None)
        if driver_worker is None or not hasattr(driver_worker, "model_runner"):
            return
        try:
            jacobian_score = driver_worker.model_runner.compute_greedy_jacobian_score(
                seq_group_meta)
        except Exception:
            logger.exception("Jacobian score computation failed for request %s",
                             request_output.request_id)
            return
        if jacobian_score is None:
            return
        setattr(request_output.outputs[0], 'jacobian_score', jacobian_score)
        setattr(request_output.outputs[0], 'jacobian_available', True)
    ...
    def _compute_single_uncertainty(self, cpl_output: CompletionOutput):
        ...
        uncertainty_dict['jacobian_score'] = getattr(cpl_output,
                                                     'jacobian_score', None)
        uncertainty_dict['jacobian_available'] = getattr(
            cpl_output, 'jacobian_available', False)
