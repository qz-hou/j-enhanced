...
def _get_and_verify_max_len(
    ...
    # HF configs in the wild use either `rope_scaling.type` (older) or
    # `rope_scaling.rope_type` (newer, e.g. some Llama-3 configs). Avoid
    # crashing on missing keys and keep the same scaling behavior.
    rope_scaling_type = None
    if isinstance(rope_scaling, dict):
        rope_scaling_type = rope_scaling.get("type") or rope_scaling.get(
            "rope_type")

    if rope_scaling is not None and rope_scaling_type != "su":
        # Some configs (e.g. certain Llama-3 exports) include `rope_scaling`
        # metadata without an explicit scaling `factor`. In that case, we
        # should not crash; just fall back to the derived max length from the
        # base config.
        if "factor" not in rope_scaling:
            return int(max_model_len) if max_model_len is not None else int(
