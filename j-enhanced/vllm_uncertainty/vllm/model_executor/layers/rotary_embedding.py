...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        ...
        # Custom CUDA kernels have no autograd; Jacobian / grad w.r.t. inputs_embeds
        # needs the PyTorch-native path.
...
def get_rope(
    ...
    if rope_scaling is None:
        ...
        # HF configs may use either `type` (older) or `rope_type` (newer).
        scaling_type = rope_scaling.get("type") or rope_scaling.get("rope_type")
        if scaling_type is None:
            raise ValueError(
                "Invalid rope_scaling config: missing 'type'/'rope_type'.")

        if scaling_type != "su":
            # Some configs include rope_scaling metadata without an explicit
            # scaling factor. In that case, fall back to the base RoPE.
            scaling_factor = rope_scaling.get("factor")
            if scaling_factor is None:
                rotary_emb = RotaryEmbedding(head_size, rotary_dim,
                                             max_position, base,
                                             is_neox_style, dtype)
